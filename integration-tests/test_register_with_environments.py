...
""" a name of an environment is taken from test_config. The param below is a position of the environment
    in a property candlepin.environment.names - it is test_config property
"""
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            id="one valid environment",
        ),
        pytest.param([0, 1], id="two valid environments"),
    ],
)
def test_register_with_org_and_environments(external_candlepin, subman, test_config, environments):
    """
    ...
    env_ids = [env_ids_in_config[ii] for ii in environments]
    env_names = [env_names_in_config[ii] for ii in environments]
...
def test_register_with_org_and_wrong_environment(external_candlepin, subman, test_config):
    ...
    with RHSMPrivateBus(proxy) as private_bus:
        ...
        with pytest.raises(DBusError) as excinfo:
            ...
            assert f'Environment with ID "{wrong_env_id}" could not be found' in str(excinfo.value)
...
def test_register_with_org_and_empty_environment(external_candlepin, subman, test_config):
    ...
    with RHSMPrivateBus(proxy) as private_bus:
        ...
        with pytest.raises(DBusError) as excinfo:
            ...
            assert 'Environment with ID "" could not be found' in str(excinfo.value)
