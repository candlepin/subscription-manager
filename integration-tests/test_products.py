...
from constants import RHSM, RHSM_PRODUCTS, TEST_PRODUCT_CERT_PATHS
from utils import loop_until, installed_products
import json
import logging
import pytest
import shutil
...
def test_products_list_installed_products(any_candlepin, subman, test_config):
    ...
    src_cert_path = first(TEST_PRODUCT_CERT_PATHS)
    ...
    def products_are_the_same(product_a: dict[str, str], product_b: dict[str, str]) -> bool:
        if len(product_a) != len(product_b):
            return False
        for key in product_a.keys():
            if product_a.get(key, "") != product_b.get(key, ""):
                return False
        return True

    assert len(products) == len(products_in_response)
    for product01, product02 in zip(
        sorted(products_in_response, key=lambda p: p.get("Product ID", "")),
        sorted(products, key=lambda p: p.get("Product ID", "")),
    ):
        assert products_are_the_same(product01, product02), f"not the same: {product01}, {product02}"
