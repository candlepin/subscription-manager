...
class DBusObjectTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_identity.cert_dir_path = (
            "path.txt"  # must be a string, otherwise it is set as a mock object
        )
    ...
    def dbus_request(self, reply_handler, proxy, proxy_args=None, error_handler=None):
        ...
        if not self.result_queue.empty():
            exc, exc_value, traceback = self.result_queue.get()
            raise exc(exc_value).with_traceback(traceback)
...
class DBusRequestThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            time.sleep(0.001)
            self.proxy(*self.proxy_args, reply_handler=self.reply_handler, error_handler=self.error_handler)
        except Exception as e:
