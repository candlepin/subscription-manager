...
class TestContainerContentUpdateActionCommand(fixture.SubManFixture):
    def setUp(self):
        ...
        self.host_cert_dir = join(self.temp_dir, "etc/docker/certs.d/")
    ...
    def _mock_cert(self, base_filename):
        cert = mock.Mock()
        cert.path = join(self.temp_dir, DUMMY_CERT_LOCATION, "%s.pem" % base_filename)
        cert.key_path.return_value = join(self.temp_dir, DUMMY_CERT_LOCATION, "%s-key.pem" % base_filename)
        return cert
    ...
    def test_multi_directory(self):
        ...
        cmd = ContainerContentUpdateActionCommand(None, [host1, host2, host3], self.host_cert_dir)
            ...
            with mock.patch(
                ...
                with mock.patch("subscription_manager.plugins.PluginManager") as mock_plugin_manager:
                    ...
                    for hostname in registry_hostnames:
                        self.assertFalse(
                            exists(join(self.host_cert_dir, hostname)),
                            "%s cert dir should not exist" % hostname,
                        )
                    ...
                    for hostname in registry_hostnames:
                        self.assertTrue(
                            exists(join(self.host_cert_dir, hostname)), "%s cert dir should exist" % hostname
                        )


class TestKeyPair(fixture.SubManFixture):
    def test_expected_filenames(self):
        kp = KeyPair("/etc/pki/entitlement/9000.pem", "/etc/pki/entitlement/9000-key.pem")
    ...
    def test_expected_filenames_weird_extensions(self):
        kp = KeyPair("/etc/pki/entitlement/9000.crt", "/etc/pki/entitlement/9000-key.crt")
        self.assertEqual("9000.cert", kp.dest_cert_filename)
        self.assertEqual("9000.key", kp.dest_key_filename)

    def test_expected_filenames_weird_filenames(self):
        kp = KeyPair("/etc/pki/entitlement/9000.1.2014-a.pem", "/etc/pki/entitlement/9000.1.2014-a-key.pem")
        self.assertEqual("9000.1.2014-a.cert", kp.dest_cert_filename)
        self.assertEqual("9000.1.2014-a.key", kp.dest_key_filename)

    def test_equality(self):
        kp = KeyPair("/etc/pki/entitlement/9000.pem", "/etc/pki/entitlement/9000-key.pem")
        kp2 = KeyPair("/etc/pki/entitlement/9000.pem", "/etc/pki/entitlement/9000-key.pem")
        self.assertEqual(kp, kp2)

    def test_inequality(self):
        kp = KeyPair("/etc/pki/entitlement/9000.pem", "/etc/pki/entitlement/9000-key.pem")
        kp2 = KeyPair("/etc/pki/entitlement/9001.pem", "/etc/pki/entitlement/9001-key.pem")
