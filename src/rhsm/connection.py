...
class BaseConnection(object):
    def __init__(
        ...
        self.conn = restlib_class(self.host, self.ssl_port, self.handler,
                                  ...
                                  ca_dir=self.ca_dir, insecure=self.insecure,
...
class ContentConnection(BaseConnection):
    def __init__(self, **kwargs):
        ...
        super(ContentConnection, self).__init__(handler='/', cert_dir='/etc/pki/entitlement', user_agent=user_agent,
                                                **kwargs)

    def get_versions(self, path, ent_cert_key_pairs=None):
        handler = "%s/%s" % (self.handler, path)
        result = self.conn.request_get(handler, cert_key_pairs=ent_cert_key_pairs)
