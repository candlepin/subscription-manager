...
class RepoFileBase(object):
    ...
    def __init__(self, path=None, name=None):
        ...
        if not self.path_exists(self.repos_dir):
            log.warning("%s does not exist, turning manage_repos off." %
                    self.repos_dir)
    ...
    def create(self):
        ...
        if not self.path_exists(self.PATH):
            log.debug('The directory %s does not exist. Trying to create it')
            try:
                os.makedirs(name=self.PATH, mode=0o755, exist_ok=True)
            except Exception as err:
                log.warning('Unable to create directory: %s, error: %s' % (self.PATH, err))
